...
- Macro and `#EXPAND` hover / go-to-definition / signature help / usage count now find a definition even when it sits in an `#ifdef`/`#ifndef` branch the current run doesn't compile — the tooling shouldn't depend on which branch is active. (Definitions inside a `{ … }` block comment are still, correctly, not offered.)
