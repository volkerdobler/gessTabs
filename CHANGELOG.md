...
- Fixed: autocomplete for a `#`-prefixed name (a macro call, or a `#` keyword like `#DEFINE`/`#MACRO`) showed "No suggestions" once anything had been typed after the `#` — e.g. `#zusatz` + `Ctrl+Space` found nothing even though the macro existed and the bare `#` alone listed it. Cause: the language's word pattern excludes `#` from word characters, so vscode's default replace range (and the filter text it's matched against) never included the `#`, while the suggestion's own label did. Completion items now carry an explicit range/filterText that starts at the `#`; a variable name or a plain (non-`#`) keyword is now also left out entirely whenever the cursor sits right after a `#`, since neither can validly follow one.
- Fixed the actual root cause behind a related symptom (`Ctrl+Space` right after typing just one character listed everything, including entries that didn't even contain that character): `wordPattern` in `language-configuration.json` required a minimum of **two** characters to recognize anything as a "word" at all (`[A-Za-zÄÖÜßäöü]([A-Za-zÄÖÜßäöü0-9_.]+)`, the trailing `+` demanding at least one more character beyond the first). A single-letter identifier — a common macro parameter or variable name (`x`, `n`, …) — was therefore never recognized as a word anywhere in the extension, not just in autocomplete; vscode's own `getWordRangeAtPosition` returned nothing for it, and every feature that falls back to vscode's default word-based filtering (autocomplete chief among them) then filtered nothing instead of filtering to that one character. Changed to `*` so a single character is now a valid word.
