...
- **Go to Matching Directive** (`Ctrl+Shift+^`, the same key as _Go to Bracket_) — on a `#MACRO`/`#ENDMACRO` line (also `#IFDEF`-family/`#END`, `#STARTEXPORT`/`#ENDEXPORT`, `IFBLOCK`/`WHILEBLOCK`…`ENDBLOCK`, `SETFILTER`/`ENDFILTER`) jumps to the matching delimiter, nesting-aware; anywhere else the key still does the built-in bracket jump.
