...
| **4** | Feed the phase-0 `externalNames.ts` output into the model as `origin: 'external'`. (The reader itself — CSV + SPSS + delimited `DATAFILE` — is pulled out to **phase 0**, see §11.) **Core model + go-to-definition done 2026-09-05** — `buildVariableModel(index, { externalNames })` seeds one `origin: 'external'` symbol per raw column, column-order preserved, promoted to `'declared'` on a real in-script declaration (never on a mere `COMPUTE`/`IF…THEN`, §3.3); `GesstabsDefintionProvider` wired. Still open: find-references/rename/Ctrl+T, and the undefined-variable diagnostic itself (phase 6) — see TODO.md P1.4/P1.6. | low | "undefined variable" diagnostic, real go-to-def for dataset vars |
