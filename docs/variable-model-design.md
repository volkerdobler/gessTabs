...
export function buildVariableModel(
  ...
  // externalNames wired 2026-09-05 (P1.4) — an already-resolved
  // ExternalNameSource[] (externalNames.ts / GesstabsExternalNamesManager;
  // this module does no filesystem I/O itself) seeds one `origin:
  // 'external'` symbol per raw column, before the program-order pass,
  // column order preserved. macroExpansion (P1.5) wired 2026-09-05 —
  // opt-in (unlike externalNames): seeds origin: 'macro-produced'
  // symbols, but no consumer passes it true yet.
...
| **1** | `variableStatements.ts` + `toLogicalStatements` + full spec. **No consumer wired yet.** **First cut landed 2026-09-04** — `src/core/statements.ts` + `src/core/variableStatements.ts` + specs; covers §3.1–§3.5, the §4 name-modes, `defKind`, and `block` open/close/mid. Open: overcodes (§3.6), the non-pattern `TO` form + `$`-member expansion (§9 Q2), `POSTPROCESS` virtuals, exhaustive per-row spec. Numeric-suffix `TO` expansion (§9 Q2) wired 2026-09-05. **`classifyTable` rewritten against the manual's full `TABLE`/`OVERVIEW`/`XOVERVIEW` grammar 2026-09-05** (`dokumentation/online-manual/md/Datenauswertung _ Kreuztabelle _ Syntax.md`) — `taboptions` (`ADD`/`NAME`/`TITLE`/`CELLELEMENTS(…)`/`FRAMEELEMENTS(…)`/`TABLEFORMATS(…)`/`CONTENTKEY`/`HIDDEN(…)`/`SORT AS`) and each part's `content [filter] [option]` (the `<cellelement>(<varname>...)` call, its `:DESCRIPTION`/`:USEVARTITLE`/`:FORMAT` suffix — confirmed from the worked examples to precede the `(…)`, not follow it — `FILTER … |` as an `ifKnown` condition, and the `SORT` option) are now parsed rather than swept by a keyword allowlist; see TODO.md P1 for the fixed false positive and the one known remaining gap (a bare `CELLELEMENT`-style keyword with no `(...)` args is still indistinguishable from a `<varname>`). `$`-member spans and `POSTPROCESS` virtuals remain open, see TODO.md. | low — pure, additive | everything below |
...
| **3** | Move go-to-definition, references, rename, semantic highlighting onto the model. Delete `regex.ts`/`matching.ts`. Move F2 empty-varlist + duplicate-declaration onto the model. **In progress 2026-09-04** — go-to-def / find-references / rename done (`collectVariableOccurrences` in `variableModel.ts`); semantic highlighting done (`collectModelSemanticTokens`, fixes the "last-name" bug); F2 empty-varlist + duplicate-declaration done (kind-gated on `defKind`, design §9 Q3); `DocumentSymbolProvider` + `symbolCompletion` done. `regex.ts`/`matching.ts`'s dead functions deleted (`findDefinitionLine`, `findAllUsages`, `lineMatchesDefinition`, `lineMatchesUsage`, `lineHasQuotedVariableReference`, the old `collectDeclarationTokens`/`collectSemanticTokens` pass, `variableInfo.ts` in full); `matching.ts` now just `matchInScope`. **Follow-up bug fixed 2026-09-05**: go-to-definition still returned every `IF…THEN`/`COMPUTE` reassignment as an alternate target, not just the real declaration(s) — `VariableSymbol.definitions` mixes both (§3.3: a reassignment is never a declaration). Fix: `VariableSymbol.definitionKinds` (parallel `defKind` array) + `primaryDefinitions(sym)` in `variableModel.ts` (declaration-kind entries when any exist, else the earliest entry), consumed by `GesstabsDefintionProvider`; see TODO.md. **`GessTabsWorkspaceSymbolProvider` (Ctrl+T) migrated 2026-09-05**: one workspace-wide `buildWorkspaceIndex`/`buildVariableModel` (every un-INCLUDEd file its own root, covering every independent entry program) feeding `primaryDefinitions(sym)` per symbol; macro/`#EXPAND`/`TABLE` names stay the existing per-document regex/classifier scan; `query` is now a substring filter over the full result instead of being compiled into the scan regex per keystroke — see TODO.md. **Follow-up over-correction fixed same day**: the F12 fix's "fall back to the single earliest entry" was too blunt — a name created by a bare `COMPUTE` once per `#ifdef`/`#else` arm was silently collapsed to one location (both in go-to-definition and Ctrl+T). Fix: new `src/core/branchPaths.ts` (`BranchPath`/`branchPathsCompatible`) + `IncludeGraphResult.branchPaths`, recorded by `resolveIncludeGraph` from the `#ifdef`/`#else`/`#end` stack it already walks; `primaryDefinitions()` now keeps an assignment-kind entry unless it's execution-compatible with (could genuinely run after) an already-kept one. Known gap: doesn't carry across an `INCLUDE` boundary — see TODO.md for the accepted-limitation writeup and full test coverage. | med-high — behaviour-visible | Tier 2 cross-INCLUDE F2 scope, Tier 3 semantic "last-name" fix |
| **4** | Feed the phase-0 `externalNames.ts` output into the model as `origin: 'external'`. (The reader itself — CSV + SPSS + delimited `DATAFILE` — is pulled out to **phase 0**, see §11.) **Done 2026-09-05** — `buildVariableModel(index, { externalNames })` seeds one `origin: 'external'` symbol per raw column, column-order preserved, promoted to `'declared'` on a real in-script declaration (never on a mere `COMPUTE`/`IF…THEN`, §3.3); go-to-definition, find-references, rename (guarded — throws rather than silently editing script text for a name the dataset itself won't rename) and Ctrl+T all wired, each via a `GesstabsExternalNamesManager` constructor param. The variable hover deliberately kept its own, separately-tuned `externalSourcesFor` fallback rather than switching to the model's `origin: 'external'` path. Still open: the undefined-variable diagnostic itself (phase 6) — see TODO.md P1.4/P1.6. | low | "undefined variable" diagnostic, real go-to-def for dataset vars |
| **5** | macro-produced names into the model (numeric/comma params, mid-token). `#DOMACRO` stays Tier 3. **Core done 2026-09-05** — `findAllMacroProducedNames` (`symbolIndex.ts`) + `buildVariableModel(index, { macroExpansion: true })` (opt-in — see TODO.md P1.5 for why). Not wired into any consumer yet; P1.6 is the motivating one. | med | — |
