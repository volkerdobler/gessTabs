...
// **bold**/*italic*/`code`/[links](url). Kept in its own pure module (no
// vscode import) so it's unit-testable like the rest of this codebase's
// parsers.
//
...
// Markdown source (and not a control character, so it can't trip an
// editor or linter that scans for those) — marks where a code span was
// pulled out of the text below.
...
// unpaired "*" inside one code span could otherwise pair up with one
// inside a *different* code span later on the same line and wrap
// everything in between (backticks included) in a bogus <em>.
...
// match GitHub's own slugger (nothing here is ever rendered by GitHub);
