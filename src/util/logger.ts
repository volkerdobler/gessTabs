// A leveled logger writing to a real vscode.OutputChannel, replacing
// printDebugMessage's console.log-gated-by-boolean — invisible to a user
// unless they open the Extension Host's dev tools, not a realistic ask
// when diagnosing a bug report. Ported from the sibling gessq extension's
// identical module (src/infra/logger.ts).

import * as vscode from 'vscode';

export type LogLevel = 'off' | 'error' | 'warn' | 'info' | 'debug';

const RANK: Record<LogLevel, number> = {
  off: 0,
  error: 1,
  warn: 2,
  info: 3,
  debug: 4,
};

let channel: vscode.OutputChannel | null = null;
let level: LogLevel = 'error';

// Falls back to `console` when no channel is registered yet, so logging
// from a unit test (no real vscode.window) never throws.
export function setOutputChannel(c?: vscode.OutputChannel): void {
  channel = c ?? null;
}

// Set the active log level explicitly (mainly for tests).
export function setLogLevel(l: LogLevel): void {
  level = l;
}

// Prefers `gesstabs.logLevel`; falls back to the pre-existing
// `gesstabs.debugMode` boolean (true -> debug, else error) so that
// setting keeps working without requiring a settings.json migration.
export function refreshLogLevelFromConfig(): void {
  try {
    const cfg = vscode.workspace.getConfiguration('gesstabs');
    const explicit = cfg.get<LogLevel>('logLevel');
    if (explicit && explicit in RANK) {
      level = explicit;
      return;
    }
    level = cfg.get<boolean>('debugMode', false) ? 'debug' : 'error';
  } catch {
    level = 'error';
  }
}

function write(msgLevel: Exclude<LogLevel, 'off'>, msg: string): void {
  if (RANK[msgLevel] > RANK[level]) {
    return;
  }
  const line = `[${msgLevel}] ${msg}`;
  if (channel) {
    channel.appendLine(line);
  } else {
    console.log(line);
  }
}

export const error = (msg: string): void => write('error', msg);
export const warn = (msg: string): void => write('warn', msg);
export const info = (msg: string): void => write('info', msg);
export const debug = (msg: string): void => write('debug', msg);
